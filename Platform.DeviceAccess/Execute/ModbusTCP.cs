...
{
    /// <summary>ModbusTCP 执行对象(与 RTU 的差别:MBAP 头替代 从站地址+CRC)</summary>
    internal class ModbusTCP : ExecuteObject
    {
        #region 匹配
        internal override Result Match(List<DevicePropEntity> devicePropList, List<TransferObject> transferObjList)
        {
            ...
            var transferObject = transferObjList.FirstOrDefault(t => t.GetType().Name == "TransferModbusTCP" && ps.All(p => t.Condition.Any(c => c == p)));
            if (transferObject == null)
            {
                TransferObject = new TransferModbusTCP();
        ...
        private List<byte> ReadGroupBytes(byte slaveId, ModbusGroupAddress group)
        {
            ...
            //根据功能码决定响应数据字节数
            int respDataLen = (group.FuncCode == FC_ReadCoils || group.FuncCode == FC_ReadDiscreteInputs)
                ? (group.Count + 7) / 8 : group.Count * 2;

            //收发
            return SendAndReceive(slaveId, pdu, respDataLen);
        }
        #endregion
        #endregion
        ...
        private byte[] ConvertEndian(byte[] deviceBytes, Type valueType, bool deviceLittleEndian, bool lowWordFirst)
        {
            ...
            int unit = valueType == typeof(bool) ? 1 : Marshal.SizeOf(valueType);//类型占字节数
            ...
            for (int s = 0; s + unit <= result.Length; s += unit)
            {
                ...
                {
                    int regCount = unit / 2;//类型占几个寄存器
                    for (int i = 0; i < regCount / 2; i++)//交换寄存器，只执行一半数量的寄存器
                    {
                        int a = s + i * 2;
        ...
        #region 收发
        private static ushort _transactionId = 0;   // 事务标识,每帧自增

        private List<byte> SendAndReceive(byte slaveId, List<byte> pdu, int respDataLen)
        {
            ushort txId = ++_transactionId;
            List<byte> frame = new List<byte>
            {
                (byte)(txId / 256), (byte)(txId % 256),   // 事务标识
                0x00, 0x00,                                // 协议标识:0 = Modbus
                (byte)((pdu.Count + 1) / 256), (byte)((pdu.Count + 1) % 256), // 长度 = 单元标识 + PDU
                slaveId                                    // 单元标识(从站地址)
            };
            frame.AddRange(pdu);
            ...
            #region 从传输层获取字节(TCP 按 MBAP 长度字段收帧,receiveLen/errorLen 不用)
            var resp = ((IRequestResponse)TransferObject).SendAndReceived(frame, 0, 0, timeout);
            #endregion

            #region 报文检验
            if (!resp.Status) throw new Exception(resp.Msg);
            if (resp.Data.Count < 9) throw new Exception("响应报文长度不足");

            // 事务标识回显校验
            if (resp.Data[0] != frame[0] || resp.Data[1] != frame[1])
                throw new Exception("事务标识不匹配");

            // 异常响应
            if (resp.Data[7] > 0x80)
            {
                byte errCode = resp.Data[8];
                string msg = Errors.ContainsKey(errCode) ? Errors[errCode] : $"未知异常 0x{errCode:X2}";
                throw new Exception($"Modbus 异常：{msg}");
            }
            #endregion

            // 读:数据从 MBAP(7) + 功能码(1) + 字节数(1) 之后开始
            return respDataLen > 0 ? resp.Data.GetRange(9, respDataLen) : new List<byte>();
        }
        #endregion
